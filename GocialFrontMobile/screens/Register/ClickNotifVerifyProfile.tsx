    ...
    return (
        <SafeAreaView className={`flex-1 ${isDarkMode ? "bg-black" : "bg-white"} justify-center items-center px-6`}>
            ...
            <View className="absolute bottom-[4rem] w-full px-6 flex-row justify-between">
                <TouchableOpacity onPress={() => navigation.goBack()} className={`flex-1 py-3 border ${isDarkMode ? "border-[#0A99FE]" : "border-[#2C5B90]"} rounded-md mr-2`}>
                    <Text className={`${isDarkMode ? "text-[#0A99FE]" : "text-[#2C5B90]"}  text-center font-medium`}>Plus tard</Text>
                </TouchableOpacity>

                <TouchableOpacity onPress={handleVerify} className={`flex-1 py-3 ${isDarkMode ? "bg-[#1F7500]" : "bg-green-700"} rounded-md ml-2`}>
