...
const RequestTypeMessageView: React.FC<TypeMessageViewProps> = ({ onClose }) => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { id: "1", text: "Hello", sender: "them", time: "23:17", date: "2025-03-06" },
    ]);
            ...
            {isMessageVisible && (
                <View className="flex-1 justify-end">
                    ...
                    <View className="flex-1 justify-center items-center p-5">
                        {/* Title */}
                        <Text className={`${isDarkMode ? "text-white" : ""} text-2xl font-bold mb-4 text-center`}>Cécile Eden souhaiterai débuter une discussion.</Text>

                        {/* Description */}
                        <Text className={`${isDarkMode ? "text-white" : ""} text-lg text-center mb-8`}>
                            Veux-tu autoriser Cécile Eden à t'envoyer des messages désormais ? Tu ne pourras répondre que si tu acceptes sa demande.
                        </Text>

                        {/* Buttons container */}
                        <View className="flex-row justify-between w-full">
                            <TouchableOpacity className="py-2 px-4 rounded-lg w-44 items-center">
                                <Text className={`${isDarkMode ? "text-[#FF4D4D]" : ""} text-lg`}>Refuser</Text>
                            </TouchableOpacity>
