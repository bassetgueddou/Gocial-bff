...
import { View, Text, Image, TouchableOpacity, ScrollView, FlatList, Dimensions, Pressable, ActivityIndicator, RefreshControl } from "react-native";
...
const HomeReal: React.FC = () => {
    ...
    const { activities, topActivities, loading, loadingMore, refreshing, error, refresh, toggleLike, loadMore, hasMore } = useActivities({ mode: 'real', filters });
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <View className="p-3">
                                        ...
                                        <Text className={`${isDarkMode ? "text-white" : "text-black"} text-sm`}>{formatDate(event.date)}</Text>
                                        <Text className={`${isDarkMode ? "text-white" : "text-black"} text-sm`}>{event.location || event.city || ''}</Text>
                                        ...
                                        <View className="flex-row items-center mt-2">
                                            ...
                                            <TouchableOpacity onPress={() => setShareModalVisible(true)} className={`${isDarkMode ? "bg-black" : "bg-[#F3F3F3]"} rounded-xl p-[0.7rem] ml-1`}>
    ...
    ), [topActivities, isDarkMode, scrollViewRef, fallbackImage, navigation, toggleLike]);
    ...
    const renderFooter = useCallback(() => {
        ...
        return null;
    }, [loadingMore, hasMore, activities.length, isDarkMode]);
