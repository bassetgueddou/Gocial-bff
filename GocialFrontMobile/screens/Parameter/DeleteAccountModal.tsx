...
const DeleteAccountModal = ({ visible, onClose, onConfirm }: { visible: boolean; onClose: () => void; onConfirm?: (password: string) => void }) => {
                ...
                <View className={`${isDarkMode ? "bg-[#1D1E20]" : "bg-white"} rounded-3xl w-[70%] px-6 py-6`}>
                    ...
                    <View className="items-center">
                        ...
                        <View className='flex-row justify-between gap-x-3 mt-2'>
                            ...
                            <TouchableOpacity onPress={() => { if (onConfirm && password) onConfirm(password); }} className={`bg-black w-[60%] py-3 rounded-full ${!password ? "opacity-50" : ""}`} disabled={!password}>
